...
out:
	...
	ts.tv_sec = event.t.sec;
	ts.tv_nsec = event.t.nsec + slave->correction;
	timespec_normalize(&ts);
	pr_debug("adding slave tstamp %ld.%09ld (source %ld.%09ld) to slave %s",
		 ts.tv_sec, ts.tv_nsec,
		 source_ts.tv_sec, source_ts.tv_nsec,
		 slave->clock->name);
